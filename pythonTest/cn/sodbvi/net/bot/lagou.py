...
def get_page(url, pn, keyword):
    ...
    if pn == 1:
        ...
        boo = 'false'
    page_data = {'first': boo, 'pn': pn, 'kd': keyword}
    r = requests.post(url=url, params=page_data, headers=headers, )
    return r.json()
...
def get_content(company_id):
    ...
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko)'
                      'Chrome/45.0.2454.86 Safari/537.36 115Browser/6.0.3',
        'Host': 'www.lagou.com',
        'Connection': 'keep-alive',
        'Origin': 'http://www.lagou.com'
    }
    req = requests.post(fin_url, headers=headers)
    return req.text
...
def save_excel(count_dict, file_name):
    book = xlsxwriter.Workbook(r'D:\pytestdir\%s.xls' % file_name)
